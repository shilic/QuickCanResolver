...
public class CanIO {
     ...
     */
    @Deprecated
    public void concurrentCanToModel(int canId, byte[] data8){
        concurrentCanDataToDbc(canId, data8);
     ...
     */
    @Deprecated
    public int[] concurrentModelToCan(int canId) {
        manager.updateModelToDbc(canId);
        return concurrentDbcToCanData(canId); // concurrentModelToCan
    }
    /**
    ...
    public byte[] concurrentSigTo64Bits(int sendId) {
        ...
        sendMsg.getSignalMap().values().parallelStream().forEach(sig ->
                instanceToBits(sendCanData,sig.getFieldValue() , sig.getStartBit(), sig.getBitLength(), sig.getFactor() , sig.getOffset() ,sig.getByteOrder())   );
        return sendCanData;
    }
     ...
     * 64 --> signal
     */
    private void bits64ToSig(byte[] data64, CanSignal sig) {
    ...
    protected void resolverCurrentValue(CanSignal sig,byte[] receiveCanData, int startBit , int bitLength , double factor , double offset ) {
        ...
        sig.setFieldValue(phyValue) ; // sig.currentValue = phyValue
